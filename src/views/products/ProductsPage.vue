...
export default class ProductsPage extends Vue {
  ...
  async filterById() {
    const id = await this.getCategoryId();
    if (id > 0) {
      this.products = this.products.filter((product) => product.categoriesId.includes(id));
    }
  }
  ...
  async getCategoryId() {
    ...
    if (this.$route.params.categoryName) {
      await fetch(`${TextConstants.connectionStr}/categories?name_like=${this.$route.params.categoryName}`)
        .then((res) => res.json())
        .then((data) => { 
          result = data[0].id;
        })
